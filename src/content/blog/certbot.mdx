...
- Task 2: The participant had to acquire the certificate from a traditional CA or use Certbot to acquire a cert and install it on the server
...
The last and the most interesting part of the study design to me was the support channel that they set up. They used this channel to counter two common issues in user studies with complex tasks: 1) participants failing early and never getting to the task of interest and 2) participants forgetting the issues they ran into when trying to complete a long procedure. In terms of participants failing early, they were able to give appropriate hints or nudges when a participant was having trouble SSH'ing into the server. Also, they were able to get immediate feedback when a participant ran into a problem because the participant would send them a message. Tiefenau et al. used a playbook to decide what help to provide to participants and what type of delay to use in responding.
...
The results regarding the time to configure TLS followed the same trend as the completion results. The median time for the Certbot task was 18 minutes compared to 65 minutes for the traditional CA task.

A finding that the authors found surprising was that participants performed well with the security and CA parts; however, they struggled with the tasks that were specific to configuring Apache, which was used as the webserver. These problems ranged from being unable to perform the ownership verification to not knowing how to enable the SSL module of Apache2.
...
In terms of framing, they did not find any difference in the behavior of participants.
...
The authors pointed out that the simplicity of Certbot is one of the key factors of its success and, most importantly, it didn't require users to have knowledge about the process.

>Certbot applied the knowledge of its experts automatically with little need for specialized knowledge, by guiding the user through the process using a dialog-like approach instead of requiring multiple commands on the command line.

The other part of its usability, as stated by the authors, was that it used safe defaults and automated several steps while only introducing two new ones.

## Wrap Up

The combination of Let's Encrypt and Certbot shows that when "administrator-centered engineering" is combined with simplicity, problems that are long considered hard to solve at scale can become significantly easier.
